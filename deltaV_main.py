...
while gameStage == 0:
    ...
    time_coefficients = [[1, 1], [100, 1], [100, 5], [100, 20], [100, 50], [100, 100], [
        100, 200], [100, 500], [200, 500], [500, 1000], [500, 2000], [500, 4000]]
    ...
    if (player != None):
        trajectory1 = Trajectory(
            objects, player, objects[3], needAutoOptimization=False)
        trajectory2 = Trajectory_old(
            objects, player, objects[3], needAutoOptimization=True)
        ...
        trajectory = Trajectory(
            objects, objects[3], objects[0], needAutoOptimization=False)
    ...
    for cObject in objects:
        rect = pg.rect.Rect((0, 0), (30, 30))
        rect.center = (0, 0)
        button1 = Button(rect, trajectory1.change_reletive_object,
                         cObject, cam, mouse_button=3, host_object=cObject)
        button2 = Button(rect, trajectory2.change_reletive_object,
                         cObject, cam, mouse_button=3, host_object=cObject)
...
while gameStage == 1:
    ...
    mainPhisMod.update_by_dt_few_times(
        time_coefficient[1], time_coefficient[0])
    ...
    if time_coefficient[1]*time_coefficient[0] < 1000:
        text1 = f1.render(
            str(time_coefficient[1]*time_coefficient[0])+"X", 1, (255, 255, 255))
    elif time_coefficient[1]*time_coefficient[0] < 1000000:
        text1 = f1.render(
            str((time_coefficient[1]*time_coefficient[0])//1000)+"kX", 1, (255, 255, 255))
    elif time_coefficient[1]*time_coefficient[0] >= 1000000:
        text1 = f1.render(
            str((time_coefficient[1]*time_coefficient[0])//1000000)+"MX", 1, (255, 255, 255))

    # вывод управления
    text2 = f1.render("Тяга: "+str(round(player.thrust, 2)),
                      1, (255, 255, 255))
    text3 = f2.render("управление: WASD - ракета, ", 1, (255, 255, 255))
    text4 = f2.render(
        "стрелки - камера, скролл - масштабирование, +/- время ", 1, (255, 255, 255))
    text5 = f2.render(
        "Цифры 1-8 - предустановленные длины траектории", 1, (255, 255, 255))
    text9 = f2.render("ЛКМ, ПКМ - привязка камеры/траектории",
                      1, (255, 255, 255))

    text6 = f2.render(
        "u - добавить длину,i - убавить длину, o - замкнуть траекторию, j - другой движок траектории", 1, (255, 255, 255))
